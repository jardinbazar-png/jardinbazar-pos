// ═══════════════════════════════════════════════════
// JARDINBAZAR POS — Conectado a Supabase Real
// ═══════════════════════════════════════════════════
...
export default function POSApp() {
  ...
  // ── REALTIME: escuchar cambios de productos ──
  useEffect(() => {
  ...
  const completeSale = async () => {
    ...
    const { data: venta, error: ventaError } = await supabase
      .from("ventas")
      .insert({ total, metodo_pago: payMethod })
      .select()
      .single();
    ...
    for (const item of cart) {
      await supabase
        .from("productos")
        .update({ existencia: item.existencia - item.qty })
        .eq("id", item.id);
    }

    setSaleCount(n => n + 1);
    setTotalVentas(n => n + total);
    setCart([]);
    setPayModal(false);
    setCashInput("");
    setSuccessMsg(`¡Venta registrada! ${fmt(total)}`);
    ...
    <div style={s.root}>
      ...
      <aside style={s.sidebar}>
        ...
        {[
          ...
          <button key={key} style={{ ...s.navBtn, ...(view === key ? s.navActive : {}) }}
            onClick={() => setView(key)}>
            <Ico path={icon} size={16} />
            {label}
          </button>
        ))}
      ...
      <main style={s.main}>
        ...
        <header style={s.topbar}>
          ...
          <div style={s.topRight}>
            <div style={s.kpi}>
              <span style={s.kpiLabel}>Ventas hoy</span>
              <span style={s.kpiVal}>{saleCount}</span>
            </div>
            <div style={s.kpi}>
              <span style={s.kpiLabel}>Total hoy</span>
              <span style={s.kpiVal}>{fmt(totalVentas)}</span>
            </div>
          ...
          <div style={s.posLayout}>
            ...
            <section style={s.prodPanel}>
              <div style={s.searchBox}>
                ...
                />
                {query && (
                  <button style={s.clearBtn} onClick={() => { setQuery(""); searchRef.current?.focus(); }}>
                    <Ico path={I.x} size={14} />
                  </button>
                )}
              </div>
            ...
            <section style={s.cartPanel}>
              <div style={s.cartHeader}>
                ...
                {cart.length > 0 && (
                  <button style={s.clearCart} onClick={() => setCart([])}>Limpiar</button>
                )}
              </div>
          ...
          <div style={s.tableWrap}>
            ...
            <div style={{ overflowY: "auto", flex: 1 }}>
              <table style={s.table}>
                <thead>
                  <tr>
                    {["Código", "Producto", "Departamento", "Costo", "Precio Venta", "Margen", "Stock", "Mín."].map(h => (
                    ...
                    .map(p => {
                      ...
                      return (
                        <tr key={p.id} style={{ background: isLow ? "#fef3c722" : "transparent" }}>
                          ...
                          <td style={{ ...s.td, fontWeight: 700, color: "#111827" }}>{fmt(p.precio_venta)}</td>
                          <td style={{ ...s.td, color: parseFloat(margin) > 20 ? "#16a34a" : "#d97706", fontWeight: 700 }}>{margin}%</td>
                          <td style={s.td}>
                            <span style={{ color: p.existencia <= 0 ? "#ef4444" : isLow ? "#f59e0b" : "#16a34a", fontWeight: 600 }}>
                              {isLow && <Ico path={I.warn} size={12} />} {p.existencia}
                            </span>
                          </td>
        ...
        {view === "caja" && (
          <div style={s.cajaWrap}>
            ...
            <div style={s.cajaNote}>
              ...
              <span>El módulo completo de cuadratura de caja (con conteo de billetes, Caja Vecina, BIP y cierre) se activa en la próxima etapa.</span>
            </div>
          </div>
        )}
        ...
        {view === "reportes" && (
          <div style={s.cajaWrap}>
            <div style={s.cajaNote}>
              ...
              <span>Los reportes con gráficos de ventas por hora, departamento y método de pago se activan en la próxima etapa.</span>
            </div>
          </div>
        )}
      </main>
          ...
          <div style={s.modal}>
            ...
            <div style={s.methods}>
              {[
                ...
                { key: "tarjeta", icon: I.card, label: "Tarjeta" },
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button
                style={{ ...s.confirmBtn, opacity: (payMethod === "efectivo" && parseFloat(cashInput || 0) < total) ? 0.4 : 1 }}
                disabled={payMethod === "efectivo" && parseFloat(cashInput || 0) < total}
                onClick={completeSale}>
                <Ico path={I.check} size={18} />
                Confirmar
              </button>
            </div>
          </div>
...
const s = {
  ...
  searchBox: { display: "flex", alignItems: "center", gap: 10, background: "#fff", border: "1.5px solid #e5e7eb", borderRadius: 8, padding: "0 14px", marginBottom: 14, flexShrink: 0, color: "#9ca3af" },
  ...
  cartHeader: { display: "flex", alignItems: "center", gap: 8, padding: "14px 16px", borderBottom: "1px solid #f3f4f6", color: "#374151", flexShrink: 0 },
  ...
  modal: { background: "#fff", borderRadius: 16, padding: 28, width: "100%", maxWidth: 380, boxShadow: "0 20px 60px #0003" },
  ...
  methods: { display: "flex", gap: 10, marginBottom: 20 },
  methodBtn: { flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, padding: "14px 8px", background: "#f9fafb", border: "1.5px solid #e5e7eb", borderRadius: 10, color: "#6b7280", fontSize: 12, fontWeight: 600, transition: "all .15s" },
